...
export const getAccessTokenFromUri = (uri) => {
  const match = uri.match(/access_token=([^\s&]+)/);
  if (!match) throw new Error("Could not extract access token from uri");
  return match[1];
};

export const getDisplayIcon = (item, screenshotModeEnabled) => {
  const imgUri = "levels" in item ? item.levels[0].displayIcon : item.displayIcon;
  if (imgUri && !screenshotModeEnabled) return imgUri;
  return null; // Return null for no image, component will handle it
};
